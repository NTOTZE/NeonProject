...
void ANPEncounterManager::InitializeEncounterData()
{
	...
	if (!EncounterData)
	{
		NP_LOG(NPLog, Error, TEXT("스테이지 [%s]의 EncounterData가 설정되어 있지 않습니다."), *SessionData.StageId.ToString());
		return;
	}
...
void ANPEncounterManager::DoSpawnOne(int32 WaveIndex, const FNPSpawnUnit UnitCopy)
{
	...
	const FNPCharacterData* MonsterData = UNPGameDataSubsystem::GetMonsterData(this, UnitCopy.MonsterId);
	UClass* MonsterClass = MonsterData ? MonsterData->CharacterClass.Get() : nullptr;
	if (!MonsterClass || !MonsterClass->IsChildOf(ANPMonsterCharacter::StaticClass()))
	{
		NP_LOG(NPLog, Error, TEXT("몬스터 [%s]의 클래스가 유효하지 않습니다."), *UnitCopy.MonsterId.ToString());
	}
	else
	{
		Spawned = GetWorld()->SpawnActorDeferred<APawn>(MonsterClass, SpawnTM, this, nullptr, ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn);
		if (Spawned)
		{
